...
async function renderDeveloperActivity(req, res, next) {
  try {
    const recentLogins = await ActivityLog.find({ action: 'user_logged_in' })
      .populate('user', 'name email city createdAt')
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();

    const recentRegistrations = await ActivityLog.find({ action: 'user_registered' })
      .populate('user', 'name email city createdAt')
      .sort({ createdAt: -1 })
      .limit(100)
      .lean();
    ...
    const latestUsers = await User.find({})
      .sort({ createdAt: -1 })
      .limit(20)
      .select('name email city createdAt xp stellaPoints')
      .lean();

    return res.render('devActivity', {
      recentLogins,
      recentRegistrations,
      latestUsers
    });
